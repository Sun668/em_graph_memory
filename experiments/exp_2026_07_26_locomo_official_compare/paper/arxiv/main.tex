...
\author{
  ...
  % Replace with real author block before camera-ready / named arXiv upload.
}
\date{}
...
We study a conversation-only \textbf{Entity--Memory (EM)} bipartite graph for LoCoMo open-domain QA.
...
System~B improves both token-F1@25 and recall\_acc@25 over A
($\Delta$F1~$=+1.49$, $\Delta$recall@$25$~$=+4.01$).
We use the joint gate $\mathrm{F1}(B)>\mathrm{F1}(A)$ and $\mathrm{recall@25}(B)\ge\mathrm{recall@25}(A)$ as the publish criterion for a method claim under the matched stack; this run satisfies it.
...
Overall gains are driven especially by single-hop (F1 $60.69\rightarrow 63.22$; recall $87.22\rightarrow 90.61$) and adversarial recall ($71.75\rightarrow 81.73$), with open-domain also improving.
...
These mixed per-category movements motivate the joint overall gate rather than claiming uniform gains on every category.
...
\paragraph{Causal reading.}
If the gain were only packaging of Memory text, B\_embed would beat A substantially---it does not.
If the gain were entities without semantics, B\_entity would approach B---it does not.
If the gain were mostly sequence edges, B\_noseq would collapse toward A---it remains clearly above A.
The data support \textbf{entity-conditioned candidate formation + dense ranking with $0.30/0.70$ fusion}, with sequence as a secondary term.
...
Ablations locate the gain in the combination of entity-conditioned candidates and dense ranking, not in entity scores alone, not in the mere presence of an EM graph file, and only partly in sequence edges.
...
\paragraph{Disallowed without caveat.}
Treating gaps vs.\ LoCoMo paper Observation/Dialog as matched-retriever wins; mixing unrelated LLM-judge scores into the main LoCoMo table; citing a different embedding lineage's F1 as this stack's headline.
