...
Figure~\ref{fig:case} sketches a typical query-time path (schematic).
For \emph{``When did Caroline attend the support group?''}, extract-v4 yields keys such as \texttt{caroline} and \texttt{support group}.
BM25 soft-match links them to Mentions-connected Memories; $\pm 1$ expansion adds neighbors; fusion ($0.30/0.70$) re-ranks the pool for the short reader.
System~A instead scores all turns by embedding alone.
...
\begin{sloppypar}
We evaluate on public LoCoMo; graphs use conversation content only, and QA labels are evaluation-only.
Code, data, and prebuilt artifacts:
\url{https://github.com/Sun668/em_graph_memory}
(runner \path{experiments/exp_2026_07_26_locomo_official_compare/run_publish_stack.py},
snapshot \texttt{v04}).
Use GPT-3.5 Turbo + text-embedding-3-small, then \texttt{build-graphs}/\texttt{A}/\texttt{B}/\texttt{ablation}/\texttt{summarize}.
The main claim is restricted to this matched stack.
\end{sloppypar}
