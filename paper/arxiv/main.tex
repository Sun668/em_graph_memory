...
\hypersetup{
  ...
  pdftitle={Entity-Memory Bipartite Graphs for Long-Conversation Dialog Retrieval on LoCoMo},
  pdfauthor={Anonymous Authors},
  pdfsubject={LoCoMo Dialog RAG; Entity-Memory retrieval},
  pdfkeywords={LoCoMo, conversational memory, RAG, knowledge graph}
}
...
\begin{abstract}
Long multi-session conversations need retrieval that is semantically flexible and grounded in entities.
...
To isolate the contribution of this retrieval structure, we rebuild the pipeline under a \textbf{matched stack}: entity extraction and answering with GPT-3.5 Turbo, embeddings with text-embedding-3-small, and a LoCoMo-style short-answer protocol (token-F1 and evidence recall).
These scores are from \emph{our} reimplementation under that stack---not numbers copied from the LoCoMo paper tables.
On LoCoMo-10 (10 conversations, 1986 QA items), EM fusion retrieval (System~B) reaches 46.48 token-F1@25 and 82.54\% recall\_acc@25, improving over a plain full-corpus Dialog embedding RAG baseline (System~A: 44.99 F1, 78.53\% recall) by +1.49 F1 points and +4.01 recall points \emph{overall}.
...
\begin{tikzpicture}[
  ...
  \node[box, fill=blue!8, text width=2.2cm] (conv) {Conversation\\dialogs / captions};
  \node[box, fill=blue!8, text width=2.3cm, right=0.32cm of conv] (graph) {EM graph\\Mem/Ent+Seq};
  \node[box, fill=orange!12, text width=2.1cm, right=0.32cm of graph] (q) {Question $q$\\keys $Q$};
  \node[box, fill=orange!12, text width=2.3cm, right=0.32cm of q] (gate) {BM25 match\\seeds ($\pm1$)};
  \node[box, fill=green!10, text width=2.25cm, below=0.7cm of gate] (dense) {Dense $S$\\on pool $\mathcal{P}$};
  \node[box, fill=green!10, text width=2.25cm, left=0.32cm of dense] (fuse) {Fusion\\$0.3E{+}0.7S$};
  \node[box, fill=gray!12, text width=2.05cm, left=0.32cm of fuse] (ans) {GPT-3.5\\short answer};
  \draw[arr] (conv) -- (graph);
  \draw[arr] (graph) -- (q);
  \draw[arr] (q) -- (gate);
  \draw[arr] (gate) -- (dense);
...
Each dialog text is normalized with a pronoun/time rewriter
(\texttt{replace\_pronouns}):
...
One per dialog turn, identified by dialog ID (stored as a \texttt{memory:}~prefixed node id), plus session index, timestamp, speaker, raw/normalized text, and optional caption.

\paragraph{Entity node $e$.}
LLM-extracted concept with type in
\{\textrm{Who}, \textrm{What}, \textrm{When}, \textrm{Where}, \textrm{Why}, \textrm{How}, \textrm{How much}\},
...
We use extract version v4 with GPT-3.5 Turbo.
The fixed prompt (scaffold length 2430 characters) asks for high-signal short noun phrases, encourages subject--predicate--object coverage when present, forbids interrogatives/function words and generic auxiliaries, and requests a JSON array of objects with \texttt{value} and \texttt{type} fields.
Extraction chat uses temperature $0.3$ and a 2500-token budget, with up to three parse retries.
The same extractor produces question entity keys $Q$ at query time (cached per sample; not stored in the conversation graph).
The builder also attaches the speaker as an entity when the package default \texttt{add\_speaker\_as\_entity} is enabled.

\subsection{Dense Memory index}
\label{sec:embed}

The embedding string for Memory $m$ concatenates normalized text, raw text, speaker, caption, and query fields.
We embed with text-embedding-3-small and score by cosine similarity against the question embedding (non-positive similarities clipped at 0 in the score map).
...
\State $Q \gets \mathrm{ExtractEntityKeys}(q)$ \Comment{GPT-3.5 Turbo, v4}
\If{$Q \neq \emptyset$}
...
\end{center}
in retrieval order.
The reader is GPT-3.5 Turbo with temperature $0$ and default 512 completion tokens (retry with larger budgets on empty outputs).
...
(\texttt{conv-26}, \texttt{30}, \texttt{41}, \texttt{42}, \texttt{43}, \texttt{44}, \texttt{47}, \texttt{48}, \texttt{49}, \texttt{50}).

Matched stack:
\begin{itemize}[leftmargin=*,itemsep=1pt]
  \item Extract / answer: GPT-3.5 Turbo
...
This is the fair plain Dialog dense RAG baseline under text-embedding-3-small.

\paragraph{System B-embed.}
...
If question entity keys are empty for System~B as well, retrieval likewise falls back to full-corpus dense scoring.
...
Table~\ref{tab:main} reports overall metrics from our matched-stack reimplementation
(again: not LoCoMo paper Table~3 scores).
System~B improves both primary \emph{overall} metrics over A
($\Delta$F1~$=+1.49$, $\Delta$recall@$25$~$=+4.01$).
Joint overall improvement indicates the retrieval structure helps without a global F1--recall tradeoff, while per-category movements can still differ (Table~\ref{tab:cat}).
...
\paragraph{Entity-only (B-entity).}
Removing dense scores collapses overall F1 to 40.78 and recall@25 to 65.99\%.
...
Under our matched stack (GPT-3.5 Turbo extract-v4, text-embedding-3-small, GPT-3.5 Turbo short answering), Entity--Memory fusion with weights $0.30/0.70$ and $\pm 1$ sequence expansion improves \emph{overall} token-F1@25 and recall\_acc@25 over plain Dialog embedding RAG on LoCoMo-10.
These are reimplementation scores under that stack, not LoCoMo paper Table~3 entries.
Ablations exclude several alternative explanations (dense packaging alone; entity scores alone; sequence-dominant gains) and are consistent with entity-conditioned candidates plus dense fusion, without claiming a unique causal pathway.
...
\begin{sloppypar}
Code, LoCoMo-10 data, prebuilt graphs/embeddings, and metric files are released at
\url{https://github.com/Sun668/em_graph_memory}.
The A/B/ablation runner is
\path{experiments/exp_2026_07_26_locomo_official_compare/run_publish_stack.py}
(snapshot \texttt{v04\_gpt35\_tes\_ab\_ablation}).
Set \texttt{OPENAI\_MODEL} to GPT-3.5 Turbo and \texttt{EM\_GRAPH\_EMBED\_MODEL} to text-embedding-3-small, then run
\texttt{build-graphs}, \texttt{A}, \texttt{B}, \texttt{ablation}, and \texttt{summarize}.
The main claim is restricted to this matched stack; gaps vs.\ LoCoMo paper Dialog/Observation rows are not matched-retriever wins.
\end{sloppypar}
...
\appendix
\section{Ungated dense retrieval (A / B-embed)}
\label{app:ungated}
When question entity keys are empty, retrieval reduces to
$\mathrm{score}(m)=S(m)$ over all Memories, returning top-$k$ dialog IDs.
This is the path used by System~A and System~B-embed (forced empty keys), and the fallback for System~B when soft-match yields no entity scores.

\section{Scope of the main claim}
\label{app:claims}
The intended main claim is restricted to the matched stack:
under GPT-3.5 Turbo + text-embedding-3-small, Entity--Memory $0.3/0.7$ fusion with sequence expansion improves LoCoMo-10 Dialog RAG token-F1@25 from 44.99 to 46.48 and recall\_acc@25 from 78.53\% to 82.54\% versus plain Dialog embedding RAG in \emph{our} reimplementation.
